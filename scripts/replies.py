...
def get_my_id(client):
    me = client.get_me()
    if me and me.data:
        return me.data.id
    sys.exit("ERROR: could not resolve my own user id")
...
def main():
    ...
    dry = os.environ.get("PRIOR_REPLIES_DRY_RUN") == "1"
    ...
    mentions = fetch_mentions(client, my_id, state.get("since_id"))
    ...
    for m in mentions:
        ...
        try:
            r = client.create_tweet(text=reply_text, in_reply_to_tweet_id=int(tid))
